...
module.exports = function (options) {
    options = options || {};
    iget.mergeGlobalConfig(options);

    var extract = iget.extract,
        user = process.env.USER || process.env.USERNAME,
        client = iget.createRemoteClient({
            auth: {
                user: user,
                password: ''
            },
    ...
    function check(languages) {
        ...
        if (!languages) {
            languages = iget.globalConfig.languages;
                ...
                igets.forEach(function (key) {
                    ...
                    languages.forEach(function (searchedLang) {
                        if (!translations[searchedLang]) {
                            missed.push(file.path + ': ' + searchedLang + ' translation required for `' + key + '`');
                        }
                    });
                });
                done();
